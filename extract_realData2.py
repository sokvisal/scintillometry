...
#test= fftshift(fft2(a_f[offsetn:offsetn+sizen,offsetm:offsetm+sizem]))
#test[test.shape[0]/2,:]=0.
#test[:,test.shape[1]/2]=0.


#plt.figure()
#plt.imshow(test.real, aspect='auto', origin='lower')
#plt.colorbar()
#plt.title(r"Conjugate Spectrum")
#plt.savefig('a.png')
#plt.close()

#a_mean = a.mean()
#a0, a1 = np.where(a==0)[0], np.where(a==0)[1]
...
b = a[offsetn:offsetn+sizen,offsetm:offsetm+sizem]
...
if pad == 1 and pad2 == 1:
    ...
    a_input[0:sizen, meff-int(round(sizem/2.)):meff] =  a_input[0:sizen, int(round(sizem/2)):sizem]
    a_input[0:sizen, int(round(sizem/2.)):sizem] = 0+0j
    plt.subplot(1,2,1)
    plt.imshow((a_input).real, aspect='auto', interpolation='nearest', origin='lower', cmap='viridis')
    
    a_input[neff-int(round(sizen/2.)):neff,0:meff] = a_input[int(round(sizen/2)):sizen, 0:meff]
...
if pad == 0 and pad2 == 0:
    ...
    plt.figure()
    plt.imshow(np.fft.fftshift(a_input).real, aspect='auto', interpolation='nearest', origin='lower', cmap='viridis')
    plt.colorbar()
    plt.title(r"Dynamic Spectrum: No Padding")
    plt.ylabel(r"Time")
    plt.xlabel(r"Freq")
    plt.savefig('dyn_spec_nopadding.png')
    plt.close()
...
epsilon=np.identity(meff_f/2)*10e-8
input_f=np.zeros(shape=(meff_f/2, sizen*meff_f/2), dtype=complex)

print a_input.shape, 'a_input'
print input_f.shape, 'input_f'
#a_input = np.flipud(a_input) 

plt.figure()
plt.imshow((a_input).real, aspect='auto', interpolation='nearest', origin='lower', cmap='viridis')
plt.colorbar()
plt.savefig('a_input.png')
plt.close()
...
for j in xrange(0,neff/2):
    ...
    print j, j*meff_f,(j+1)*meff_f, meff_f
    input_f[0:meff_f/2,j*meff_f/2:(j+1)*meff_f/2] = sp.linalg.toeplitz(cols,rows)
    print input_f, j
    if j==0:
        print 0,meff_f/2,j*meff_f/2,(j+1)*meff_f/2
        print meff-const, pad2*meff*0+const
        input_f[0:meff_f/2,j*meff_f/2:(j+1)*meff_f/2] = sp.linalg.toeplitz(np.conj(np.append(a_input[j,:meff-const],np.zeros(pad2*meff*0+const))))+epsilon
        print input_f, j
print "##########################"
if neff == 1:
    neff -= 1
#print input_f[:40,:40], 'blocked matrix'
input_f = input_f
print input_f
#if pad == 0 and pad2 == 0:
#    input_f = input_f[:sizen,:sizem]
        
tar = tarfile.open('data.tar.gz','w:gz')
for rank in xrange(0,nump):
    size_node_temp=(sizen//nump)*meff_f/2
    size_node=size_node_temp
    if rank==nump-1:
        size_node = (sizen//nump)*meff_f /2+ (sizen%nump)*meff_f/2
...
output = np.memmap(output_file, dtype='complex', mode='w+', shape=(meff_f/2, sizen*meff_f/2),order='F')
